...
function years_option(){
  ...
  select_year.append('input')
            ...
            .attr('step', '1')
            .attr("value", "2020")
            .on("click", function() {update(+this.value);});
...
function update_bottom(radio_selection) {
  console.log(radio_selection);
  d3.select('#graph').remove();
  d3.select("#"+ String(radio_selection)).property("checked", true);
  generate_pies(glob_year, radio_selection);
...
function radio_bottom(){
 ...
 var radio1 = bottom.append('input')
            ...
            .on("change", function() {console.log(this.id); update_bottom(+this.id);});
  ...
  var radio2 = bottom.append('input')
            ...
            .attr('name', 'Cycle of study')
            .attr('value', 'Master')
            .on("change", function() {update_bottom(+this.id);});
...
Promise.all([
    ...
    // iscrizioni = iscrizioni.filter(function(d){
    //   if(isNaN(d.value)){
    //     return false;
    //   }
